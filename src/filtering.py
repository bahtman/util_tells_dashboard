...
@st.cache_data
def tab_data(df, map, throwerBuy, opponentBuy):
    df = df[(df['map_name'] == map) & df['throwerBuyType'].isin(throwerBuy) & df['opponentBuyType'].isin(opponentBuy)].reset_index(drop=True)
